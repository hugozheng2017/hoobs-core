...
const { once } = require("hap-nodejs/lib/util/once");
...
module.exports = class Server {
    constructor(opts) {
        ...
        this._api.on("registerPlatformAccessories", (accessories) => {
            this._handleRegisterPlatformAccessories(accessories);
        });
    
        this._api.on("updatePlatformAccessories", (accessories) => {
            this._handleUpdatePlatformAccessories(accessories);
        });
    
        this._api.on("unregisterPlatformAccessories", (accessories) => {
            this._handleUnregisterPlatformAccessories(accessories);
        });
    
        this._api.on("publishExternalAccessories", (accessories) => {
        ...
        this._setupManager = new Manager();
    
        this._setupManager.on("newConfig", () => {
            this._handleNewConfig()
        });
    
        this._setupManager.on("requestCurrentConfig", (callback) => {
    ...
    _publish() {
        ...
        this._bridge.on("listening", (port) => {
            _system.info(`Bridge is running on port ${port}.`);
        });
    ...
    _loadPlugins() {
        ...
        Plugin.installed().forEach((plugin) => {
            ...
            try {
                ...
                _system.error(`Error loading plugin "${plugin.name()}".`);
                _system.error(err.stack);
            ...
            if (!plugin.loadError) {
                ...
                _system.info(`Loaded plugin "${plugin.name()}".`);
    ...
    _loadConfig() {
        ...
        if (!existsSync(configPath)) {
            _system.warn(`config.json "${configPath}" not found.`);
        ...
        try {
            ...
            _system.error("There was a problem reading your config.json file.");
        ...
        if (config.ports !== undefined) {
            if (config.ports.start > config.ports.end) {
                _system.error("Invalid port pool configuration. End should be greater than or equal to start.");
        ...
        _system.info(`Loaded config.json with ${(config.accessories && config.accessories.length) || 0} accessories and ${(config.platforms && config.platforms.length) || 0} platforms.`);
    ...
    _loadAccessories() {
        _system.info(`Loading ${this._config.accessories.length} accessories...`);
    
        for (let i = 0; i < this._config.accessories.length; i++) {
            const accessoryConfig = this._config.accessories[i];
            const accessoryConstructor = this._api.accessory(accessoryConfig["accessory"]);
            ...
            this._bridge.addBridgedAccessory(this._createAccessory(accessoryInstance, accessoryConfig["name"], accessoryConfig["accessory"], accessoryConfig.uuid_base));
        }
    }
    
    _loadPlatforms() {
        _system.info(`Loading ${this._config.platforms.length} platforms...`);
    
        for (let i = 0; i < this._config.platforms.length; i++) {
            const platformConfig = this._config.platforms[i];
            const platformConstructor = this._api.platform(platformConfig["platform"]);
            ...
            if (platformInstance.configureAccessory == undefined) {
                this._loadPlatformAccessories(platformInstance, platformLogger, platformConfig["platform"]);
            } else {
                this._activeDynamicPlugins[platformConfig["platform"]] = platformInstance;
            }
    
            if (platformInstance.configurationRequestHandler != undefined) {
                this._configurablePlatformPlugins[platformConfig["platform"]] = platformInstance;
            }
        }
    }
    ...
    _configCachedPlatformAccessories() {
        ...
        for (let index in this._cachedPlatformAccessories) {
            ...
            let platformInstance = this._activeDynamicPlugins[accessory._associatedPlugin + "." + accessory._associatedPlatform];
    ...
    _loadPlatformAccessories(platformInstance, _system, platformType) {
        this._asyncCalls++;
        ...
        platformInstance.accessories(once((foundAccessories) => {
            ...
            for (let i = 0; i < foundAccessories.length; i++) {
                ...
                _system(`Initializing platform accessory "${accessoryInstance.name}"...`);
    ...
    _createAccessory(accessoryInstance, displayName, accessoryType, uuid_base) {
    ...
    _handleRegisterPlatformAccessories(accessories) {
    ...
    _handleUpdatePlatformAccessories() {
        this._updateCachedAccessories();
    }
    
    _handleUnregisterPlatformAccessories(accessories) {
        const hapAccessories = [];
    ...
    _handlePublishExternalAccessories(accessories) {
        for (let index in accessories) {
            const accessory = accessories[index];
            ...
            if (this._externalPorts) {
                if (this._nextExternalPort > this._externalPorts.end) {
                    _system.info("External port pool ran out of ports. Fallback to random assign.");
            ...
            const advertiseAddress = this._generateAddress(accessory.UUID);
            ...
            ((name) => {
                hapAccessory.on("listening", (port) => {
                    _system.info(`${name} is running on port ${port}.`);
                });
    ...
    _handleNewConfig(type, name, replace, config) {
        if (type === "accessory") {
            if (!this._config.accessories) {
            ...
            if (!replace) {
                ...
                for (let index in this._config.accessories) {
                    if (this._config.accessories[index].accessory === name) {
                    ...
                    if (targetName && (this._config.accessories[index].accessory === targetName)) {
                        this._config.accessories[index] = config;
            ...
            if (!replace) {
                ...
                for (let index in this._config.platforms) {
                    if (this._config.platforms[index].platform === name) {
                    ...
                    if (targetName && (this._config.platforms[index].platform === targetName)) {
                        this._config.platforms[index] = config;
        ...
        writeFileSync(User.configPath(), JSON.stringify(this._config, null, 4), "utf8");
    }
